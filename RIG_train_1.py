...
from def_module import generate_valid_positions_BlackKingForced, generate_valid_initial_position_BlackKingForced, board_to_tensor, generate_legal_actions_mask, decoder_action, select_action_BlackKing
...
vae_state_collection_update_size = 5000
vae_num_epochs = 200
vae_batch_size = 100
vae_num_epochs_update = 100
vae_batch_size_update = 100
beta = 0.5
...
exploration_states = generate_valid_positions_BlackKingForced(vae_state_collection_size)          # Collect D using various positions (1.)
...
for episode in range(num_episodes):
    ...
    board = generate_valid_initial_position_BlackKingForced()          # Sample initial state (6.)
    ...
    if episode % update_vae_every == 0 and len(replay_buffer) > vae_state_collection_update_size:
        states_for_update = generate_state_collection_update(replay_buffer, exploration_states, vae_state_collection_update_size, mixture_coeff)
...
torch.save(vae.state_dict(), "vae_1.pth")
torch.save(policy.state_dict(), "policy_1.pth")
print("Apprentissage du VAE et de la politique terminé et enregistré dans", "vae_1.pth", " et", "policy_1.pth")